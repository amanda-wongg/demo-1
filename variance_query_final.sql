with date_params as (
  select 
    dateadd(day, -(date_part(dow, current_date) + 6) % 7, current_date)::date as start_date,
    dateadd(day, -(date_part(dow, current_date) + 2) % 7, current_date)::date as end_date
),
cal as 
(
  select
...
from
  ...
  left join (
    ...
    where
      date = (select start_date - interval '1 day' from date_params)
