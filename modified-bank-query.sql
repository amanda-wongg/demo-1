-- Modified Bank Reconciliation Query
-- Automatically uses previous week's Monday-Friday dates
-- Run this every Monday morning to get last week's data
...
payment_record_movment as (
  ...
  from
    ...
    cross join date_params dp  -- Join with our calculated dates
  where
    ...
    -- Use calculated previous week dates instead of parameters
...
select
  ...
  -- Show the actual dates being used for transparency
  ...
  -- Add a summary row showing the date range for clarity
...
from
  ...
  left join (
    ...
    where
      date = (select dateadd(day, -1, prev_monday) from date_params)  -- Day before previous Monday
  ...
  left join (
    ...
    from
      rolled_up_daily_bt_totals
    where
      date = (select prev_friday from date_params)  -- Previous Friday
