...
with date_params as (
  ...
  select 
    case 
      ...
      when datepart(weekday, getdate()) = 2 then dateadd(day, -7, dateadd(day, -(datepart(weekday, getdate()) - 2), getdate()))
      -- For any other day, find the Monday of the previous week
      else dateadd(day, -(datepart(weekday, getdate()) + 5), getdate())
    end as start_date,
    
    case 
      -- If today is Monday, go back to previous week's Friday
      when datepart(weekday, getdate()) = 2 then dateadd(day, -3, dateadd(day, -(datepart(weekday, getdate()) - 2), getdate()))
      -- For any other day, find the Friday of the previous week
      else dateadd(day, -(datepart(weekday, getdate()) + 1), getdate())
    end as end_date
),
