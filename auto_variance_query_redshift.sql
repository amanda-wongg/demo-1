...
-- Redshift version - No manual date parameters needed!
...
(
  select
    ...
    bnpc.business_day,
    bnpc.bank_account,
    nvl(abt.daily_bt_amount, 0) daily_bt_amount
  from
...
payment_record_movment as (
  ...
  from
    ...
    date_params dp  -- Use our calculated dates
  where
...
what_is_unreconciled as (
  SELECT
    ...
    SUM(nvl(bt.credit_amount, 0)) credit_total,
    SUM(nvl(bt.debit_amount, 0)) debit_total,
    (nvl(debit_total, 0) - nvl(credit_total, 0)) as absolute_total
  FROM
...
select
  ...
  nvl(bb1.date, dp.start_date) bt_date_1,
  nvl(bb1.daily_balance, 0.00) bt_balance_1,
  bb2.date bt_date_2,
  bb2.daily_balance bt_balance_2,
  bt_balance_2 - bt_balance_1 as movement,
  movement - reconciled_payment_records as difference,
  nvl(wiu.credit_total,0) as unreconciled_bt_credit,
  nvl(wiu.debit_total, 0) as unreconciled_bt_debit,
  nvl(wiu.absolute_total, 0) unreconciled_bt_absolute_total, 
  movement - reconciled_payment_records - nvl(wiu.absolute_total, 0) as variance
from
  date_params dp,
  payment_record_movment prm
  left join (
    select
      *
    from
